...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, fp16, strides=[10, 20, 40]):
        super().__init__()
    ...
    def forward(self, inputs, labels=None):
        ...
        for kl, (stride, output) in enumerate(zip(self.strides, inputs)):
            output, grid = self.get_output_and_grid(output, kl, stride)
            x_shifts.append(grid[:, :, 0])
            y_shifts.append(grid[:, :, 1])
            expanded_strides.append(torch.ones_like(grid[:, :, 0]) * stride)
            outputs.append(output)

        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels,
                               torch.cat(outputs, 1))
    ...
    def get_output_and_grid(self, output, k, stride):
        ...
        if grid.shape[2:4] != output.shape[2:4]:
            yv, xv          = torch.meshgrid([torch.arange(hsize),
                                              torch.arange(wsize)])
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        n_candidate_k           = min(6, pair_wise_ious.size(1))
        topk_ious, _            = torch.topk(pair_wise_ious, n_candidate_k, dim=1)
        dynamic_ks              = torch.clamp(topk_ious.sum(1).int(), min=1)  # 动态设置候选框数量
...
def copy_attr(a, b, include=(), exclude=()):
    for kk, v in b.__dict__.items():
        if (len(include) and kk not in include) or kk.startswith('_') or kk in exclude:
            continue
        else:
            setattr(a, kk, v)
...
class ModelEMA:
    ...
    def update(self, model):
        ...
        with torch.no_grad():
            ...
            for kk, v in self.ema.state_dict().items():
