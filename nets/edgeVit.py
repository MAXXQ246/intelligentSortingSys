...
class EdgeVit(nn.Module):
    ...
    """
    def __init__(self, depth=[1, 2, 5, 3], img_size=640, in_chans=3, num_classes=2, embed_dim=[128, 256, 512, 1024],
        ...
        self.patch_embed1 = PatchEmbed(
            img_size=img_size, patch_size=5, in_chans=in_chans, embed_dim=embed_dim[0])
        self.patch_embed2 = PatchEmbed(
            img_size=img_size // 5, patch_size=2, in_chans=embed_dim[0], embed_dim=embed_dim[1])
        self.patch_embed3 = PatchEmbed(
            img_size=img_size // 10, patch_size=2, in_chans=embed_dim[1], embed_dim=embed_dim[2])
        self.patch_embed4 = PatchEmbed(
            img_size=img_size // 20, patch_size=2, in_chans=embed_dim[2], embed_dim=embed_dim[3])
    ...
    def forward(self, x):
        ...
        feature4x = x
        for blk in self.blocks1:
            feature8x = blk(feature4x)
        feature16x = self.patch_embed2(feature8x)
        for blk in self.blocks2:
            feature16x = blk(feature16x)
        feature32x = self.patch_embed3(feature16x)
        for blk in self.blocks3:
            feature32x = blk(feature32x)
        feature32x2 = self.patch_embed4(feature32x)
        for blk in self.blocks4:
            feature32x2 = blk(feature32x2)
